...
├── account.txt              # AppState local (ignoré par Git)
│
├── config/
...
>
> Ton AppState reste donc sur ta machine. Pour le déploiement, Render le reçoit via la
> variable d'environnement `FB_APPSTATE` (étape 5) — jamais via Git.
...
3. **`account.txt`** (fichier local, développement — ignoré par Git).
