...
export const SECONDARY_NPCS = Object.freeze([
  ...
  },
  {
    ...
    kind: "artisan",
    zone: "académie",
    // Academy yard, not the Yamanaka residential district.
    position: [-44, 0.55, 14],
  },
  {
    id: "river_toma",
...
export async function ensureSecondaryMissions(db, now = Date.now()) {
  ...
  for (let slot = 0; slot < SECONDARY_SLOTS; slot++) {
    let current = mutable.find((row) => Number(row.slot) === slot);
    const npc = current ? npcById(current.npc_id) : null;
    // The academy giver was moved from a residential plot into the academy
    // yard. Keep already-persisted assignments usable by migrating only the
    // private return point; mission identity and progress stay unchanged.
    if (current && npc) {
      const savedObjective = JSON.parse(current.objective || "{}");
      const expectedReturn = JSON.stringify([...npc.position]);
      if (
        current.zone !== npc.zone ||
        JSON.stringify(savedObjective.returnPosition) !== expectedReturn
      ) {
        savedObjective.returnPosition = [...npc.position];
        const updated = rowTime();
        const objective = JSON.stringify(savedObjective);
        await db
          .prepare(
            "UPDATE secondary_missions SET zone=?,objective=?,updated=? WHERE slot=? AND mission_id=?",
          )
          .run(npc.zone, objective, updated, current.slot, current.mission_id);
        current = { ...current, zone: npc.zone, objective, updated };
        mutable[mutable.findIndex((row) => Number(row.slot) === slot)] =
          current;
      }
    }
...
export class SecondaryMissionService {
  ...
  async refreshPeer(peer) {
    const row = await this.db
      ...
      .get(peer.id);
    peer.secondaryUnlocked = secondaryUnlocked(row);
    if (peer.secondaryUnlocked && !this.hasUnlockedPlayer) await this.refresh();
