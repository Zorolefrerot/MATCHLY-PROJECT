...
export function createApp(db) {
  const getQuiz = () => {
    const row = db.prepare("SELECT value FROM settings WHERE key='quiz'").get();
  ...
  const cleanup = setInterval(() => {
    ...
    db.prepare("DELETE FROM resets WHERE expires<?").run(now);
  }, 60000);
  cleanup.unref();
  ...
  function session(req, res, user) {
    ...
    res.cookie("iz_session", token, { ...cookieOptions, maxAge: 7 * 86400000 });
  }
  app.use("/api", (req, res, next) => {
  ...
  const admin = (req, res, next) =>
    req.user?.role === "admin"
      ...
      : res
          .status(403)
          .json({ error: "Cet espace est réservé au propriétaire." });
  const validEmail = (e) =>
    typeof e === "string" &&
    e.length <= 254 &&
    /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e);
  ...
  app.get("/api/quiz", (req, res) =>
    res.json(getQuiz().map(({ answer, ...q }) => q)),
  );
    ...
    if (user) {
      ...
      try {
        await transport.sendMail({
          ...
          subject: "IDREM ZENKAI — Réinitialisation du mot de passe",
          text: `Ce lien est valable 30 minutes : ${process.env.PUBLIC_ORIGIN}/reinitialiser?token=${token}\nSi tu n’es pas à l’origine de cette demande, ignore ce message.`,
        });
      } catch {
        db.prepare("DELETE FROM resets WHERE token=?").run(digest(token));
  ...
  app.post("/api/auth/reset", limit, (req, res) => {
  ...
  app.post("/api/application", auth, limit, (req, res) => {
    if (req.user.role === "admin")
      return res
        .status(400)
        .json({ error: "Le propriétaire ne consomme pas de place de joueur." });
    const lengths = {
      character: [3, 50],
      discovery: [5, 500],
      goals: [20, 1500],
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Réponds aux 10 questions du quiz." });
    if (req.body.consent !== true)
      return res
        .status(400)
        .json({ error: "Tu dois accepter les règles de candidature." });
    if (
      db.prepare("SELECT id FROM applications WHERE user_id=?").get(req.user.id)
    )
      return res
        .status(409)
        .json({ error: "Tu as déjà envoyé une candidature." });
  ...
  app.post("/api/allocation", auth, (req, res) => {
    try {
      res.json(allocate(db, req.user.id));
    } catch (e) {
  ...
  app.post("/api/admin/applications/:id", auth, admin, (req, res) => {
    if (
  ...
  app.get("/api/admin/quiz", auth, admin, (req, res) => res.json(getQuiz()));
  app.put("/api/admin/quiz", auth, admin, (req, res) => {
