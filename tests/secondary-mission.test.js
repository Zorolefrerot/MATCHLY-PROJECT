...
test("secondary missions are global, locked until clan reward, and always have three zones", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      SECONDARY_NPCS.find((npc) => npc.id === "academy_doctor").position,
      [-44, 0.55, 14],
      "the academy giver is placed in the academy yard",
    );
    assert.equal(
    ...
    const stalePeer = { id: 1, secondaryUnlocked: false };
    assert.equal((await service.refreshPeer(stalePeer)).unlocked, true);
...
test("the academy giver migration repairs old private return coordinates", async () => {
  const { db, dir } = await fixture();
  try {
    const service = new SecondaryMissionService(db);
    await service.refresh();
    const row = await db
      .prepare("SELECT * FROM secondary_missions WHERE slot=0")
      .get();
    const objective = JSON.parse(row.objective);
    objective.returnPosition = [-60, 0.55, -8];
    await db
      .prepare(
        "UPDATE secondary_missions SET npc_id='academy_doctor',zone='ancien quartier',objective=? WHERE slot=0",
      )
      .run(JSON.stringify(objective));
    await service.refresh();
    const repaired = await db
      .prepare(
        "SELECT npc_id,zone,objective FROM secondary_missions WHERE slot=0",
      )
      .get();
    assert.equal(repaired.npc_id, "academy_doctor");
    assert.equal(repaired.zone, "académie");
    assert.deepEqual(
      JSON.parse(repaired.objective).returnPosition,
      [-44, 0.55, 14],
    );
  } finally {
    await db.close();
