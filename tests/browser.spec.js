...
test("home, clan filters, FAQ and mobile layout", async ({ page }) => {
  ...
  ).toContainText("TA VOIE NINJA");
  await page
    .getByRole("button", {
      name: "Tous les clans · 14",
    })
    .click();
  ...
  await page
    .getByText("Est-ce que je peux déjà jouer sur Android ?", {
      exact: true,
    })
    .click();
  await expect(
...
test("candidate registration, quiz, admin decision and stable heritage", async ({
  ...
  await page
    .getByRole("link", {
      name: "Commencer ma candidature",
    })
    .click();
  await page
    .getByLabel("Où as-tu découvert IDREM ZENKAI ?", {
      exact: false,
    })
    .fill("Un ami m’a parlé de cette communauté.");
  await page
    .getByLabel("Que souhaites-tu faire dans le jeu ?", {
      exact: false,
    })
    .fill("Je souhaite participer à des missions avec mes amis.");
  await page
    .getByLabel("Qu’est-ce qui te motive à nous rejoindre ?", {
      exact: false,
    })
  ...
  await page
    ...
    .click();
  await page
    .getByLabel("Nom du personnage", {
      exact: false,
    })
    .fill("Haru Shinobi");
  await page
    .getByLabel("Son histoire en quelques mots", {
      exact: false,
    })
    .fill(
