...
**Mise à jour du 14 septembre 2026 — APK 0.12.0/code 12 vérifiée :** l’exécution GitHub Actions **34829507927** a réussi l’import Godot, les **312 assertions**, les quatre scénarios WSS natifs, l’export Android et les contrôles de paquet. La résidence utilise maintenant une façade extérieure fermée et réparée : la porte est solide, un sceau bleu au sol sert de zone d’attente de trois secondes, puis une image de chargement s’affiche avant la téléportation vers l’intérieur virtuel. Aucun bouton d’entrée ou de sortie n’est utilisé ; la sortie se fait en marchant vers le seuil intérieur. L’intérieur conserve ses deux niveaux, ses salles utiles, ses portraits encadrés et son balcon orienté vers Konoha. L’installateur public est [`idrem-zenkai-training-debug.apk`](https://github.com/Zorolefrerot/MATCHLY-PROJECT/releases/download/android-latest/idrem-zenkai-training-debug.apk), **72 911 948 octets**, sans connexion GitHub nécessaire. Le site continue de vérifier l’admission avant de fournir le lien. Le site pointe vers cette compilation pour les joueurs acceptés ; il faut encore déclencher le Manual Deploy du service Render pour rendre la nouvelle référence visible en production.
...
- Uniquement `Authorization: Bearer …` de la session native. Cookie du site/propriétaire, jeton en URL, candidat non admis et session expirée refusés. Toute candidature acceptée avec un rôle joueur peut entrer dans Konoha ; l’attribution du clan est facultative pour l’accès initial.
...
`server/android-build.js` pointe maintenant vers l’APK publique **0.12.0/code 12** du village complet avec la façade fermée réparée, le sceau bleu de trois secondes et le chargement avant téléportation sans bouton : release permanente [`android-latest`](https://github.com/Zorolefrerot/MATCHLY-PROJECT/releases/download/android-latest/idrem-zenkai-training-debug.apk), **72 911 948 octets**. La route du serveur vérifie toujours que le compte est accepté avant de rediriger ; la release publique évite le 404 des artefacts Actions et ne demande pas de compte GitHub.
