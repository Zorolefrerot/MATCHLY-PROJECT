...
**Mise à jour du 14 septembre 2026 — APK 0.12.0/code 12 vérifiée :** l’exécution GitHub Actions **34825077814** a réussi l’import Godot, les **312 assertions**, les quatre scénarios WSS natifs, l’export Android et les contrôles de paquet. La résidence utilise maintenant un hall extérieur ouvert en permanence et l’interaction générique `E` pour entrer et sortir ; l’intérieur conserve ses deux niveaux, ses salles utiles, ses portraits encadrés et son balcon orienté vers Konoha. L’artefact téléchargeable est [`idrem-zenkai-android-85`](https://github.com/Zorolefrerot/MATCHLY-PROJECT/actions/runs/34825077814/artifacts/10339672702), **79 898 738 octets**, jusqu’au **21 septembre 2026 à 08:56:35 UTC**. Le site pointe vers cette compilation pour les joueurs acceptés ; il faut encore déclencher le Manual Deploy du service Render pour rendre la nouvelle référence visible en production.
...
`server/android-build.js` pointe maintenant vers l’APK **0.12.0/code 12** vérifiée du village complet avec le hall ouvert de la résidence du Hokage et l’entrée manuelle par `E` : ZIP `idrem-zenkai-android-85`, **79 898 738 octets**, expiration **21 septembre 2026 à 08:56:35 UTC**. Le téléchargement reste protégé par l’admission du compte ; la nouvelle référence sera visible en production après le Manual Deploy Render.
