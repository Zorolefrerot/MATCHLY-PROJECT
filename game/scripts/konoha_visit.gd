...
func _academy_hint() -> String:
	if not is_instance_valid(secondary_manager):
		return ""
	...
	if status in ["IN_PROGRESS", "TIME_EXPIRED", "REPORT_PENDING"]:
		return "\n\nLes missions secondaires seront débloquées après le rapport et la récompense de ta deuxième mission auprès de ton chef de clan."
	return "\n\nCette académie est d’abord le panneau de la mission d’accueil d’Aoi. Les missions secondaires apparaissent après la deuxième mission de clan et sa récompense."
...
func _sync_mission() -> void:
	...
	if is_instance_valid(secondary_manager):
		if secondary_value is Dictionary:
...
func _mission_response(operation: String, success: bool, message: String) -> void:
	...
	if operation == "clanMission":
		...
		if event == "report":
			...
			menu_event = ""
			_sync_mission()
			# The village socket cached the unlock state when the visit started.
			# Revalidate it immediately after the reward report so the academy
			# secondary giver is usable without leaving and re-entering Konoha.
			if is_instance_valid(village_link): village_link.secondary_refresh()
			hud.hide_menu()
