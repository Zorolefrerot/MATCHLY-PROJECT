...
func run() -> void:
	...
	for frame in range(180):
		await physics_frame
	Input.action_release("move_forward")
	check(not visit.exterior.outside and visit.player.position.z < 160.5, "the real south route returns to Konoha through the gate (z=%0.2f outside=%s)" % [visit.player.position.z, str(visit.exterior.outside)])
